...
bool Calculator::run(void)
{
    ...
    int w = 924;
    int h = (getVisualArea().y2 - getVisualArea().y1) / 4 * 3;

    // lv_obj_t *keyboard = lv_obj_create(lv_scr_act());
    // lv_obj_center(keyboard);
    // lv_obj_set_size(keyboard, w, h);
    // lv_obj_set_style_bg_color(keyboard, lv_color_make(0x00, 0x00, 0x00), 0);
...
static string get_event_code_name(lv_event_code_t code)
{
    switch (code) {
    case LV_EVENT_ALL:
        return "LV_EVENT_ALL";
...
void Calculator::keyboard_event_cb(lv_event_t *e)
{
    ...
    // ESP_UI_LOGE("%s", get_event_code_name(code).c_str());
